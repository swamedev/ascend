...
**ARCHITECTURE_MODE:** strict-execution  
**PROJECT:** Competency Development Framework (CDF)  
**CREATED:** 2026-07-19  
**LAST_UPDATED:** 2026-07-19T01:43:00-03:00  
**PHASE:** Foundation & Architecture Complete → Strict Execution Era (Sprint 7)
...
### ADR-028: Architectural Foundation Closure & Protocol Adoption
- **Date:** 2026-07-19
- **Status:** Accepted
- **Context:** A Operação Hermes e Odyssey foi concluída, gerando o ARCH-0010 (Policy Engine) e os protocolos fundamentais (PROTOCOL-0001 a 0005). A fase de documentação estrutural está encerrada. O código atual precisa começar a materializar esses contratos, especificamente para corrigir a Violação V1 (desbloqueio de competência sem evidência).
- **Decision:** Declarar o congelamento da fundação arquitetural. Iniciar a "Era da Execução Estrita". Implementar o `Policy Protocol` (`src/ascend/application/policies.py`) e instanciar a `CompetencyPolicies.can_unlock_competency` para proteger o sistema contra a Violação V1 (Invariante I2), atuando como a primeira barreira do Policy Engine.
- **Consequences:** O ASCEND deixa de ser um projeto de software e passa a ser um ecossistema governado por protocolos. Nenhuma regra de autorização ou transição de estado será feita via `if` solto no código; tudo passará pelo Policy Engine retornando um `PolicyDecision` formal.
- **Hash:** `adr028-foundation-closure-policy-engine-20260719`

### ADR-029: Separation of Powers (Services vs. Policies) & Assessment Service
- **Date:** 2026-07-19
- **Status:** Accepted
- **Context:** A Dívida Técnica Crítica apontada na auditoria v1.0 indicava a ausência do `AssessmentService` para o comando `CompleteAssessment`. Além disso, a "Era da Execução Estrita" exige que os Serviços de Aplicação não tomem decisões de autorização sozinhos.
- **Decision:** Criar o `AssessmentService` e a `AssessmentPolicies`. Estabelecer o padrão arquitetural de Separação de Poderes: Services (Poder Executivo) executam mutações de estado apenas após receberem um `PolicyDecision.ALLOW` do Policy Engine (Poder Judiciário).
- **Consequences:** O sistema ganha um endpoint seguro para que Agentes de IA (via AAP) submetam avaliações. O código torna-se auditável e blindado contra transições de estado inválidas (ex: reavaliar uma evidência já aceita).
- **Hash:** `adr029-separation-of-powers-assessment-20260719`
...
| 2026-07-19 | ADR-028 | **MILESTONE** — Architectural Foundation Closure & Protocol Adoption |
| 2026-07-19 | ADR-029 | Criação — Separation of Powers (Services vs. Policies) & Assessment Service |
